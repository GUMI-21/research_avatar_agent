...
class MessageRecord(Base):
    ...
    agent_id: Mapped[str] = mapped_column(ForeignKey("agents.id"))  # 实际处理消息的 Agent
    run_id: Mapped[str | None] = mapped_column(
        String(36), nullable=True, index=True
    )  # 产生该消息的执行；用户消息可为空
    role: Mapped[str] = mapped_column(String(16))  # LLM 协议中的消息类型
    content: Mapped[str] = mapped_column(Text)  # 最终正文，不保存流式 delta
