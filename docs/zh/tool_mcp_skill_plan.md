...
| 风险 | 示例 | 默认策略 |
| --- | --- | --- |
| read_only | 读取允许目录内文件 | 可自动执行 |
| local_write | 修改或创建 Server 文件 | 每次审批 |
| external_read | 读取网页、邮件或日程 | 首版每次审批，后续允许用户配置 |
| external_write | 发邮件、创建日程、提交网页表单 | 每次审批 |

所有参数先经过 Pydantic 严格校验。文件路径解析真实路径后必须位于管理员配置的允许根目录。工具事件只记录工具名、风险、状态、耗时和安全摘要，不记录密钥、邮件正文或完整文件内容。
...
2. Server 文件只读工具与 Native Agent 多轮工具循环。
